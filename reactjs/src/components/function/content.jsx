...
export const timeAgo = (date) => {
    ...
    if (diffYears > 0) {
      return `${diffYears} năm ${remainingMonths} tháng ${remainingWeeks} tuần ${remainingDays} ngày ${remainingHours} giờ ${remainingMinutes} phút ${remainingSeconds} giây trước `;
    }
    if (diffMonths > 0) {
      return `${diffMonths} tháng ${remainingWeeks} tuần ${remainingDays} ngày ${remainingHours} giờ ${remainingMinutes} phút ${remainingSeconds} giây trước`;
    } 
    if (diffWeeks > 0) {
      return `${diffWeeks} tuần ${remainingDays} ngày ${remainingHours} giờ ${remainingMinutes} phút ${remainingSeconds} giây trước`;
    } 
    if (diffDays > 0) {
